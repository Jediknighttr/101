// server.js — net hata/uyarı mesajları + güvenli başlatma + log
...
function countHumans(r){ return Object.values(r.players).filter(p=>!p.isBot).length; }
function roomSummary(r){
  return {
    id:r.id, settings:r.settings, seats:r.seats, hostId:r.hostId,
...
function clearTable(r){
  ...
  r.hands={S:[],W:[],N:[],E:[]}; r.turn='S'; r.started=false;
}
function startGame(r){
  ...
  console.log(`[ROOM ${r.id}] START | gösterge=${r.gosterge.renk} ${r.gosterge.sayi} | okey=${r.okey.renk} ${r.okey.sayi} | turn=${r.turn}`);
  io.to(r.id).emit('toast', 'Oyun başladı. İyi şanslar!');
...
io.on('connection',(socket)=>{
  ...
  console.log(`[ROOM ${roomId}] CONNECT ${socket.id}`);
  ...
  socket.on('join',({name})=>{
    ...
    if(!room.hostId) room.hostId=pid; // ilk insan host
    socket.data.playerId=pid;

    if (room.started) {
      room.needsRestart = true;
      io.to(roomId).emit('toast','Yeni canlı oyuncu katıldı. Host oyunu yeniden başlatabilir.');
    }

    console.log(`[ROOM ${roomId}] JOIN ${name||'Oyuncu'} seat=${empty} host=${room.hostId}`);
    io.to(roomId).emit('state', roomSummary(room));
  });

  socket.on('chooseSettings',({mode,bot,turnSeconds})=>{
    const pid=socket.data.playerId;
    if(room.hostId!==pid){ socket.emit('errorMsg','Sadece host ayar yapabilir.'); return; }
    if(room.started){ socket.emit('errorMsg','Oyun başladıktan sonra ayar değişmez.'); return; }
    ...
    console.log(`[ROOM ${roomId}] SETTINGS mode=${room.settings.mode} bot=${room.settings.bot} t=${room.settings.turnSeconds}`);
    io.to(roomId).emit('state', roomSummary(room));
  });

  socket.on('startOrFill', ()=>{
    const pid=socket.data.playerId;
    if(room.hostId!==pid){ socket.emit('errorMsg','Sadece host başlatabilir.'); return; }
    if(room.started){ socket.emit('errorMsg','Oyun zaten başlamış.'); return; }
    fillBots(room);
    console.log(`[ROOM ${roomId}] START CMD by host=${pid} -> filling bots & deal`);
    startGame(room);
  });

  socket.on('restartGame', ()=>{
    const pid=socket.data.playerId;
    if(room.hostId!==pid){ socket.emit('errorMsg','Sadece host yeniden başlatabilir.'); return; }
    console.log(`[ROOM ${roomId}] RESTART by host=${pid}`);
    startGame(room);
  });
  ...
  socket.on('chatPreset',({text})=>{
    ...
    io.to(roomId).emit('toast', `${p.name}: ${String(text).slice(0,64)}`);
    io.to(roomId).emit('chat',{from:p.name,text:String(text).slice(0,64),ts:Date.now()});
  });

  socket.on('disconnect',()=>{
    const pid=socket.data.playerId;
    console.log(`[ROOM ${roomId}] DISCONNECT ${socket.id}`);
