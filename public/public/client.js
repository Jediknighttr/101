(function () {
  ...
  function showError(msg){ if(errEl){ errEl.textContent = msg; errEl.classList.remove("hidden"); } console.warn("[UI MSG]", msg); }
  ...
  socket.on("connect_error", (err)=>{ showError("Sunucuya bağlanılamadı (connect_error)."); console.error("connect_error:", err); });
  socket.on("disconnect", (reason)=>{ showError("Bağlantı koptu: " + reason); });

  // Sunucudan bilgi mesajı (başladı/sohbet vb.)
  socket.on("toast", (msg)=> showError(msg));
  ...
  joinBtn.addEventListener("click", ()=>{
    ...
    showError("Masaya katıldın. Host isen üstteki panelden başlatabilirsin.");
  });
  applySettings.addEventListener("click", ()=>{
    ...
    showError("Ayarlar gönderildi.");
  });
  fillStart.addEventListener("click", ()=>{
    socket.emit("startOrFill");
    showError("Başlat komutu gönderildi. Dağıtım bekleniyor…");
  });
  restartBtn.addEventListener("click", ()=>{
    if(confirm("Oyunu yeniden başlatmak istiyor musun?")) {
      socket.emit("restartGame");
      showError("Yeniden başlatma istendi…");
    }
  });
